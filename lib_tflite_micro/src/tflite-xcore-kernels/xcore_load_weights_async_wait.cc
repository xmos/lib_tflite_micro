...
#define MAX_OUTPUTS 4
    ...
    // printf("\ntarget addr = %08x ", data_ptrs[0]);

    // for(int i=0;i<data_sizes_in_words[0]*4; i++) {
    //   ((int8_t*)data_ptrs[0])[i] = i;
    // }
...
TFLMRegistration *Register_XC_ld_weights_async() {
  static TFLMRegistration r = {load_weights_async::Init, nullptr,
                               load_weights_async::Prepare, load_weights_async::Eval};
  return &r;
}

TFLMRegistration *Register_XC_ld_weights_wait() {
  static TFLMRegistration r = {nullptr, nullptr,
                               nullptr, load_weights_async::Eval_Wait};
  return &r;
}
