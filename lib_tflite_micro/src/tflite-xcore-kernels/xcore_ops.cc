...
void RegisterXCOps(MicroOpResolver *res) {
  ...
  resolver->AddCustom(XC_ld_weights_async_OpCode, Register_XC_ld_weights_async());
  resolver->AddCustom(XC_ld_weights_wait_OpCode, Register_XC_ld_weights_wait());
  ...
  resolver->AddCustom(XC_transpose_OpCode, Register_XC_transpose());
