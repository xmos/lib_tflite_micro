...
# Resize
im_rgb = cv2.resize(im, (input_shape[1], input_shape[2]))

cv2.imwrite("im.ppm", im_rgb, (32, 0))

# uint8 to int8
input_array = im_rgb.astype(np.float32)
input_array -= 128.0
input_array = input_array.astype(np.int8)
input_data = np.expand_dims(input_array, axis=0)

input_data.tofile("in.csv", sep=',')
add_newlines("in.csv", "in_newlines.csv")

#input_data = np.clip(input_data, 100, 100)
...
quant_params = interpreter.get_output_details()[0][
...
for i in range(0, len(detections)):
    if detections[i] != -128:
        print("%d %d " % (i, detections[i]))
# print(quant_params)
# detections = dequantize(detections, quant_params["scales"], quant_params["zero_points"])

# print(repr(detections.flatten()))
